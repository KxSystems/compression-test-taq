...
function generate_HDB () {
  if [[ ${FORMAT} == "parquet" ]]; then
    ...
    python3 ./pysrc/parquet.py -date $DATE -src $CSVDIR -dst $DSTPARQUET/uncompressed -letters $LETTERS
  else
    echo "Generating kdb+ data (aka. HDB)..."
    $QEXEC ./src/tq.q -date $DATE -src $CSVDIR -dst $DSTKDB/zd0_0_0 -letters $LETTERS -s $COMPUTECOUNT -q
  fi
}
