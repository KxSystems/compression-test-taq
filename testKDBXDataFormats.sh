...
function execute_queries () {
    ...
    for s in "${THREAD_NRS[@]}"; do
        ...
        if [[ "${SCOPE}" == *"inmem"* ]]; then
            ...
            $(get_numa_config) $QEXEC ./src/runQueries.q ${COMMONPARAMS} -db ${DB_DIR}/kdb -format kdbinmemorygrouped -engine sql -queryfile ./artifacts/queries/sql_inmemory.psv -result ${RESULT_DIR}/sqlInMemoryGrouped_${s}Threads.psv -s ${s}
