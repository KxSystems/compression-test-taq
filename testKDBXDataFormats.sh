...
function execute_queries () {
    ...
    for s in "${THREAD_NRS[@]}"; do
        ...
        if [[ "${SCOPE}" == *"inmem"* ]]; then
            ...
            DUCKDB_THREADS=$(( s > 1 ? s : 1 )) $(get_numa_config) python3 pysrc/run_queries.py ${COMMONPARAMS} -db ${DB_DIR}/parquet/rowgroup -engine duckdb_con_inmemory_index -queryfile ./artifacts/queries/inmemory/duckdb.psv -result ${RESULT_DIR}/duckdbInMemoryIndex_${s}Threads.psv
