...
function execute_queries () {
    ...
    for s in "${THREAD_NRS[@]}"; do
        ...
        $(get_numa_config) $QEXEC ${query_runner} -db ${DB_DIR}/kdb -format kdbinmemory -queryfile ./artifacts/queries/kdb.psv -result ${RESULT_DIR}/kdbInMemory_${s}Threads.psv -s ${s}
    done
}
