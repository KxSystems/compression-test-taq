...
runQuery "select from quote where ", SYMBOLCOLNAME, " = anInfreqSym, Time within 0D08:30 0D16:30";       / All data from one symbol
...
runQuery "select date, Symbol, Time, TradePrice, TradeVolume, TradeStopStockIndicator, SaleCondition, Exchange from trade where date=min date, SourceofTrade ",  / selected fields only
  $[PARQUET; "~\\: enlist \"N\""; "= \"N\""];
runQuery "select Symbol, Time, MidPrice: (BidPrice + OfferSize) %2 from select from quote where ", SYMBOLCOLNAME, " = aFreqSym";
runQuery "select date, Symbol, Time, BidPrice, OfferPrice, BidSize, OfferSize, QuoteCondition, Exchange from quote where ", SYMBOLCOLNAME, " = aFreqSym";

runQuery "select avgMidSize: avg (BidPrice + OfferPrice) % 2 from quote where ", SYMBOLCOLNAME, " = anInfreqSym";
runQuery "select avgSpread: avg OfferPrice - BidPrice by 0D00:10 xbar Time from quote where ", SYMBOLCOLNAME, " = aFreqSym";
...
runQuery "select inbal: (BidSize - OfferSize) % BidSize + OfferSize by ", / no dot notation in parquet yet
   $[PARQUET; "minute: `minute$0D00:01 xbar Time"; "Time.minute"], " from quote where ", SYMBOLCOLNAME, " = anInfreqSym";

runQuery "raze {select date, Symbol, Time, BidPrice, OfferPrice, BidSize, OfferSize, QuoteCondition, Exchange from quote where ", SYMBOLCOLNAME, " = x} each infreqIdList";
runQuery "raze {select date, Symbol, Time, BidPrice, OfferPrice, BidSize, OfferSize, QuoteCondition, Exchange from quote where ", SYMBOLCOLNAME, " = x} peach infreqIdList";
runQuery "raze {select date, Symbol, Time, BidPrice, OfferPrice, BidSize, OfferSize, QuoteCondition, Exchange from quote where ", SYMBOLCOLNAME, " = x, 2000<BidSize+OfferSize} peach infreqIdList";

runQuery "raze {select first Symbol, wsumAsk:OfferSize wsum OfferPrice, wsumBid: BidPrice wsum BidSize, sdevask: sdev OfferSize, sdevbid:sdev BidPrice, corPrice:OfferPrice cor BidPrice, corSize: OfferSize cor BidSize from quote where ", SYMBOLCOLNAME, " = x} each infreqIdList";
